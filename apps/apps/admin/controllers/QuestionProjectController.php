...
use apps\admin\models\Question as adminQuestion;
...
class QuestionProjectController extends Controller
{
    ...
    public function actionAnswerExport($id = 0, $project_id = 0, $project_region_id = 0, $keywords = '', $is_chose = 0){
        ...
        $questionData = QuestionAnswer::find()
            ->where(['question_project_id' => $id, 'status' => \apps\admin\models\Question::STATUS_ACTIVE])
        ...
        $titleArr =  Question::find()
            ->where(['status' => \apps\admin\models\Question::STATUS_ACTIVE])
            ->andWhere(['<>', 'type_id', \apps\admin\models\Question::TYPE_DEVELOP])
            ->orderBy('id','asc')->select(['site'])->all();
...
//question
    ...
    public function actionExportDevelopAnswer($year=null, $season=null){
        ...
        $titleArr =  Question::find()
            ->where(['status' => adminQuestion::STATUS_ACTIVE, 'type_id' => adminQuestion::TYPE_DEVELOP])
            ->orderBy('id','asc')->select(['site'])->all();
