...
<?= $form->field($model, 'group')->dropDownList(\common\models\ButlerAuth::groupMap()) ?>
<?= $form->field($model, 'region')->hiddenInput(['id' => 'regionIds'])->label(false) ?>
...
<script>
    ...
    $('#add_region').bind('click',function(){
        ...
        $('<li/>').html(s).attr('data-id',id).append(rm).appendTo($('#region_show'));
        updateRegionData();
