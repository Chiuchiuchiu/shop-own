...
<header class="header">
    <div class="headerFixed">
        <ul class="top-nav dis-tab">
            <li class="dis-cell"><a href="?type=0"<?php if($type==0){?> class="on" <?php }?>>全部</a></li>
                <li class="dis-cell"><a href="?type=1"<?php if($type==1){?> class="on" <?php }?>>电表</a></li>
                <li class="dis-cell"><a href="?type=2"<?php if($type==2){?> class="on" <?php }?>>冷水表</a></li>
...
<article class="signlist">
    <ul>
        <?php foreach ($MeterList as $Lib) { ?>
            <li>
                <a href="<?= $Lib->status==2 ? '/meter/create?id='.$Lib->id : 'javascript:;'; ?>">
                    <div class="dis-tab s-head">
                        <span class="dis-cell gray-6">设备编号:<?=$Lib->uid;?> </span>
                    ...
                    <div class="s-body clearfix">
                        <div class="detail">
                            <p class="name">设备类型<?=$Lib->meter_type;?></p>
                            <p class="name">上期抄表时间:<?=date('Y-m-d',$Lib->last_meter_time);?></p>
                            <p class="name">上期抄表读数:<?=$Lib->last_meter_data;?></p>
                            <p class="name"><?=$Lib->ancestor_name;?></p>
                        </div>
                    </div>
                </a>
                <div class="s-footer clearfix">
                    <p class="vuedate">设备编号:<?=$Lib->uid;?></p>
                    <?php if($Lib->status==2){?>
                    <p><a href="/meter/create?id=<?=$Lib->id;?>" style="width: 60px; height: 20px;" class="btn btn-orange right">抄表</a></p>
                    <?php }?>
                </div>
            </li>
        <?php } ?>
