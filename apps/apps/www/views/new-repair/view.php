        ...
        <div class="new-info">
            <h4>
                ...
                <?php if($model->status == \common\models\Repair::STATUS_WAIT){?>
                    <button class="re-view-cancel" data-go="/new-repair/cancel?id=<?= $model->id ?>">取消</button>
                <?php }?>

                <?php if($model->status == \common\models\Repair::STATUS_DONE){?>
                    <button data-go="/new-repair/customer-evaluation?id=<?= $model->id ?>" class="re-view-done">评价服务</button>
                <?php }?>
