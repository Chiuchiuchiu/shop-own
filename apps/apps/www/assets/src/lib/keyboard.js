...
;(function(exports){
    var KeyBoard = function(input, options){
        ...
        cssStr += '</style>';
        //Button
        var btnStr = '<div id="keyboardCompile" style="width:60px;background-color:#1FB9FF;';
        ...
        tableStr += '<tr><td style="background-color:#D3D9DF;">X</td><td>0</td>';
        tableStr += '<td style="background-color:#D3D9DF;">删除</td></tr>';
        tableStr += '</table>';
        this.el.innerHTML = cssStr + btnStr + tableStr;
        if($('#cdj-body')){
            $('#cdj-body').css({paddingBottom:'220px'});
            $('#cdj-body').scrollTop($('#cdj-body').scrollTop()+220);
        }
        function addEvent(e){
            ...
            if(clickEl.tagName.toLocaleLowerCase() === 'td' && value !== "删除"){
                if(self.input){
                    self.input.val(self.input.val()+value)
                }
            }else if(clickEl.tagName.toLocaleLowerCase() === 'div' && value === "完成"){
                $('body').trigger('closeKeyboard');
