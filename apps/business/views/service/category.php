...
\components\inTemplate\widgets\ActiveForm::begin(['layout' => 'horizontal', 'method' => 'get', 'action' => \yii\helpers\Url::toRoute('/service/category')]);
?>
    <div class="form-group">
        <div class="col-sm-2">
            <div class="input-group m-b">
                <?= Html::dropDownList('status', $status,array_merge(['' => '全部'], ProjectServiceCategory::statusMap()) )?>
            </div>
        </div>
        <div class="col-sm-2">
...
<?php
echo \components\inTemplate\widgets\Html::a('新增', ['category-create', '_referrer' => Yii::$app->request->url], ['class' => 'btn btn-success pull-right']);
?>
    ...
    'content'=>\components\inTemplate\widgets\GridView::widget([
        ...
        'columns' => [
            ...
            [
                'label' => '所属项目',
                'format' => 'raw',
                'value' => function(ProjectServiceCategory $model){
                    $name =  isset($model->projectInfo->house_name) ? $model->projectInfo->house_name : '全部';
                    return Html::label($name, '', ['class' => 'text-warning']);
                }
            ],
            [
                'class' => \components\inTemplate\widgets\RBACActionColumn::className(),
                'template'=>'{category-update?id} {setStatus}',
                'name'=>[
                    'category-update'=>'编辑'
                ],
                'buttons' => [
                    'setStatus' => function ($key,ProjectServiceCategory $model, $index){
                        return Html::a(
    ...
    $('.setStatus').click(function(){
        ...
        layer.confirm("您确定"+ name +"该分类？", {
            ...
            function(){
                ...
                $.ajax({
                    type: 'get',
                    url: "/service/set-status?id=" + id + "&status=" + status + "&type=" + 2,
