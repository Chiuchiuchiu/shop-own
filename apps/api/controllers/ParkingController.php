...
class ParkingController extends Controller
{
    public function actionIndex($projectId)
    {
        ...
        $projectParkInfo = ProjectParkingOneToOne::findOne(['project_house_id' => $projectId]);
        $postData = [
        ...
        if(!$parkingInfoResult){
            ...
            ParkingLog::writeLog($this->userId, $parkingInfoResult, $errors['msg'], $errors['code']);
            return $this->render('result', [
                'msg' => '服务异常，无车场信息！',
                'aChart' => '返回首页',
                'href' => '/'
        ...
        $memberPlate = MemberCar::find()->where(['member_id' => $this->userId])->all();
    ...
    public function actionMonthBill($parkingId, $plateNo,int $month=1)
    {
        ...
        if(empty($monthFeeVal)){
            ParkingLog::writeLog($this->userId, $postData, $monthFee->errMsg, $monthFee->errCode);
            return $this->render('result');
        }

        ParkingLog::writeLog($this->userId, $monthFeeVal, '-', '-');
    ...
    public function actionBillSubmit()
    {
        ...
        if($result){
            ...
            if($model->save())
            {
                ParkingLog::writeLog($this->userId, $result, '创建临卡订单', 'gen', $plateno);
    ...
    public function actionIrainIndex()
    {
        ...
        $viewData = [
            'parkingName' => $projectParkInfo->project->house_name,
            'parkingId' => $projectParkInfo->parking_id,
    ...
    public function actionIrainTempBill($plateNo=null)
    {
        ...
        if(isset($res['data']) && is_array($res['data'])){
            ...
            //记录日志
            ParkingLog::writeLog($this->userId, $res, '获取临卡订单', 'irain', $plateNo);

            return $this->render('irain-temp-bill', ['data' => $data, 'payType' => 't']);
        }

        ParkingLog::writeLog($this->userId, $postData, $res['message'], $res['code'], $plateNo);
    ...
    public function actionIrainCreateTempOrder()
    {
        if($this->isPost){
            ...
            ParkingLog::writeLog($this->userId, $res, '创建临卡订单', 'irain', $plateNo);
            ...
            $model->project_house_id = $projectId;
            $model->amount = $unpaid;
            ...
            ParkingLog::writeLog($this->userId, $model->getErrors(), $res['message'], $res['code']);
    ...
    {
        if($this->isAjax && isset($this->projectId)){
            $projectParkInfo = ProjectParkingOneToOne::findOne(['project_house_id' => $this->projectId]);
            ...
            return $this->renderJsonSuccess(200, ['goUrl' => $url]);
        }
    ...
    public function actionDelPlate()
    {
        ...
        $model = MemberCar::findOne(['id' => $id, 'member_id' => $this->userId]);
        if(!$model){
