...
<div class="panel" id="new-repair-list" data-max-page="<?= $dataProvider->pagination->pageCount ?>">
    <div class="topPath">
        ...
        <?php if($flowStyleID == 'w'){?>
            <a href="<?= $hasHouse ? '/repair?flowStyleID=w&site=' . $site : 'javascript:void(0)'; ?>"
               class="addNew maintain <?= $hasHouse ? '' : 'alterWin' ?>">报修</a>
        <?php } else {?>
            <a href="<?= $hasHouse ? '/repair?flowStyleID=8&site=' . $site : 'javascript:void(0)'; ?>"
    ...
    <div class="ui-row-flex tab-list">
        <div>
            <a href="/repair/list?status=0&flowStyleID=<?= $flowStyleID ?>&site=<?= $site ?>" <?= $status == '0' ? 'class="hover"' : '' ?>>待处理</a>
        </div>
        <div>
            <a href="/repair/list?status=1,2,5,6,8,9&flowStyleID=<?= $flowStyleID ?>&site=<?= $site ?>" <?= $status == '1,2,5,6,8,9' ? 'class="hover"' : '' ?>>处理中</a>
        </div>
        <div>
            <a href="/repair/list?status=3,7&flowStyleID=<?= $flowStyleID ?>&site=<?= $site ?>" <?= $status == '3,7' ? 'class="hover"' : '' ?>>待评价</a>
        </div>
        <div>
            <a href="/repair/list?status=3000&flowStyleID=<?= $flowStyleID ?>&site=<?= $site ?>" <?= $status == '3000' ? 'class="hover"' : '' ?>>已完成</a>
        </div>
        <div>
            <a href="/repair/list?status=4,1000&flowStyleID=<?= $flowStyleID ?>&site=<?= $site ?>" <?= $status == '4,1000' ? 'class="hover"' : '' ?>>已取消</a>
        </div>
    </div>
    <?php
...
<div id="bottom-nav">
    ...
    <a href="/repair/list?status=0&site=<?= $site ?>">
...
<script type="text/javascript">
    $('#new-repair-list').on('loaded', function () {
        app.scrollLoad($('#new-repair-list'), '/repair/list?status=<?= $status ?>&flowStyleID=<?= $flowStyleID ?>');
