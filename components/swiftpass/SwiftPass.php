...
class SwiftPass
{
    ...
    public function Request($config)
    {
        ...
        $this->reqHandler->setGateUrl($this->gateUrl);
    ...
    public function submitOrderInfo($subOpenId, $outTradeNo, $money, $notifyUrl, $option=[])
    {
        if(isset($option['test'])){
            $this->reqHandler->setSubOpenId('');
            $this->reqHandler->setParameter('sub_appid', '');
        } else {
...
//        $this->reqHandler->setParameter('version', $this->cfg['version']);
        ...
        if ($this->pay->call()) {
            ...
            return  $this->pay->resContent;
        }

        return ['msg'=>$this->pay->resContent];
    }
