...
class QYWechatSDK
{
    ...
    const GET_ACCESS_TOKEN_URL = '/cgi-bin/gettoken?';

    //发消息
    const SEND_MSG_URL = '/cgi-bin/message/send?';

    ///多客服相关地址
    const CUSTOM_SERVICE_GET_RECORD = '/customservice/getrecord?';
    const CUSTOM_SERVICE_GET_KFLIST = '/customservice/getkflist?';
    const CUSTOM_SERVICE_GET_ONLINEKFLIST = '/customservice/getonlinekflist?';
    const API_BASE_URL_PREFIX = 'https://qyapi.weixin.qq.com'; //以下API接口URL需要使用此前缀
    const OAUTH_TOKEN_URL = '/sns/oauth2/access_token?';
    const OAUTH_REFRESH_URL = '/sns/oauth2/refresh_token?';
    const OAUTH_USERINFO_URL = '/cgi-bin/user/getuserinfo?';
    ...
    public function getAccessToken(){
        $res = \Yii::$app->cache->get(self::className().'getAccessToken');
        if(!isset($res['expires_in']) || $res['expires_in']<time()){
            $res = $this->http_get(self::API_BASE_URL_PREFIX.self::GET_ACCESS_TOKEN_URL.'corpid='.$this->appid.'&corpsecret='.$this->appsecret);
            $res = json_decode($res,true);
            if(isset($res['expires_in'])){
                $res['expires_in']+=time()-100;
                \Yii::$app->cache->set(self::className().'getAccessToken',$res);
            }
        }
...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    {
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::SEND_MSG_URL . 'access_token=' . $this->getAccessToken(), self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @return string
     */
    public function getOauthRedirect($callback, $state = '')
    {
        return self::OAUTH_PREFIX . self::OAUTH_AUTHORIZE_URL . 'appid=' . $this->appid . '&redirect_uri=' . urlencode($callback) . '&response_type=code&scope=snsapi_base&state=' . $state . '#wechat_redirect';
    }

    public function getLoginRedirect($callback, $state = '1')
    {
        return 'https://qy.weixin.qq.com/cgi-bin/loginpage?corp_id=' . $this->appid . '&redirect_uri=' . urlencode($callback) . '&state=' . $state . '&usertype=member';
    }
    ...
    public function getOauthUserInfo()
    {
        ...
        $result = $this->http_get(self::API_BASE_URL_PREFIX . self::OAUTH_USERINFO_URL . 'access_token=' . $this->getAccessToken() . '&code='.$code);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    {
        $result = $this->http_get(self::API_BASE_URL_PREFIX.'/cgi-bin/user/get?access_token='.$this->getAccessToken().'&userid='.$userid);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
