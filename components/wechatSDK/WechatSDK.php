...
use components\wechatSDK\lib\WxPayApi;
use components\wechatSDK\lib\WxPayJsApiPay;
use components\wechatSDK\lib\WxPayUnifiedOrder;
use components\wechatSDK\lib\WxRedPackData;
...
class WechatSDK
{
    ...
    const INVOICE_URL_PREFIX = 'https://api.weixin.qq.com/card/invoice';
...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    /**
     * 删除wx_card授权TICKET
     * @author HQM 2018-09-18
     * @param string $appid 用于多个appid时使用
     * @return bool
     */
    public function resetWxcardTicket($appid = '')
    {
        if (!$appid) $appid = $this->appid;
        $this->wxcard_ticket = '';
        $authname = 'wechat_wxcard_ticket' . $appid;
        $this->removeCache($authname);
        return true;
    }
     ...
     * @author HQM 2018-09-18
     * @param string $appid 用于多个appid时使用,可空
     * @param string $wxcardTicket 手动指定wxcard_ticket，非必要情况不建议用
     * @return string|bool
     */
    public function getWxcardTicket($appid = '', $wxcardTicket = '')
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
        if (!$appid) $appid = $this->appid;
        if ($wxcardTicket) { //手动指定token，优先使用
            $this->wxcard_ticket = $wxcardTicket;
            return $this->wxcard_ticket;
        }
        $authname = 'wechat_wxcard_ticket' . $appid;
        if ($rs = $this->getCache($authname)) {
            $this->wxcard_ticket = $rs;
            return $rs;
        }
        ...
        if ($result) {
            ...
            $this->wxcard_ticket = $json['ticket'];
            $expire = $json['expires_in'] ? intval($json['expires_in']) - 100 : 3600;
            $this->setCache($authname, $this->wxcard_ticket, $expire);
            return $this->wxcard_ticket;
        }
     ...
     * @return bool|mixed
     */
    public function invoiceSetBizattr($data)
    {
        $setBizattrUrl = '/setbizattr?action=set_contact&';
        if (!$this->access_token && !$this->checkAuth()) return false;
        //
        $result = $this->http_post(self::INVOICE_URL_PREFIX . $setBizattrUrl . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @return bool|mixed
     */
    public function invoiceCreateCard($data)
    {
        $createCardUrl = '/platform/createcard?';
        if (!$this->access_token && !$this->checkAuth()) return false;
        //
        $result = $this->http_post(self::INVOICE_URL_PREFIX . $createCardUrl . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function invoiceInsert($data)
    {
        ...
        if (!$this->access_token && !$this->checkAuth()) return false;
        //
        $result = $this->http_post(self::INVOICE_URL_PREFIX . $insertUrl . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    /**
     * 电子发票：获取授权页链接
     * @author HQM 2018-09-19
     * @param $data
     * @return bool|mixed
     */
    public function invoiceGetAuthurl($data)
    {
        $authUrl = '/getauthurl?';
        if (!$this->access_token && !$this->checkAuth()) return false;
        //
        $result = $this->http_post(self::INVOICE_URL_PREFIX . $authUrl . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @return bool|mixed
     */
    public function invoiceSetPdf($data)
    {
        $setpdfUrl = '/platform/setpdf?';
        if (!$this->access_token && !$this->checkAuth()) return false;
        //
        $result = $this->http_post(self::INVOICE_URL_PREFIX . $setpdfUrl . 'access_token=' . $this->access_token, $data, true);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function invoiceGetpdf($data)
    {
        $getpdfUrl = '/platform/getpdf?action=get_url&';
        if (!$this->access_token && !$this->checkAuth()) return false;
        //
        $result = $this->http_post(self::INVOICE_URL_PREFIX . $getpdfUrl . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function uploadMedia($data, $type)
    {
        ...
        if (!function_exists('curl_file_create')) {
            //兼容PHP版本少于5.5的写法
            function curl_file_create($media)
            {
                return '@' . $media;
            }
        }
        $data['media'] = curl_file_create($data['media']);
    ...
    {
        if (is_null($openid)) $openid = $this->getRevFrom();
        if (!$this->access_token && !$this->checkAuth()) return false;
     ...
     */
    public function wxJsApiPay($outTradeNo,$openId,$money,$option=[])
    {
        require_once "lib/WxPayData.php";
        $input = new WxPayUnifiedOrder();
        //普通商户小程序
        if(isset($option['mpAppId'])){
            $input->SetAppid($option['mpAppId']);
        } else {
            $input->SetAppid(\Yii::$app->params['wxPay']['appId']);
        }

        if(isset($option['subMchId'])){
            $input->SetSubMch_id($option['subMchId']);
            $input->SetMch_id(\Yii::$app->params['wxPay']['parentMchId']);
        }else{
            $input->SetMch_id(\Yii::$app->params['wxPay']['mchId']);
        }

        //服务商户小程序
        if(isset($option['sub_appid'])){
            $input->SetSubAppid($option['sub_appid']);
        }

        $input->SetBody(isset($option['body'])?$option['body']:'缴费');
        $input->SetAttach(isset($option['attach'])?$option['attach']:'缴费');
        $input->SetOut_trade_no($outTradeNo);
        $input->SetTotal_fee($money*100);
        $input->SetTime_start(date("YmdHis"));
        $input->SetTime_expire(date("YmdHis", time() + 600));
        $input->SetGoods_tag(isset($option['goodsTag'])?$option['goodsTag']:'缴费');
        $input->SetNotify_url(isset($option['notifyUrl'])?$option['notifyUrl']:(\Yii::$app->request->hostInfo."/pay/wx-notify"));
        $input->SetTrade_type("JSAPI");
        $input->SetOpenid($openId);
        $unifiedOrder = WxPayApi::unifiedOrder($input);

        return (new WxPayJsApiPay())->GetJsApiParameters($unifiedOrder);
    }

    public static function sendRedPack($openId,$mch_billno,$amount,$num,$act_name,$wishing,$remark=''){
        require_once "lib/WxPayData.php";
        $obj = new WxRedPackData();
        $obj->setValues('mch_billno',$mch_billno);
        $obj->setValues('mch_id',\Yii::$app->params['wxPay']['mchId']);
        $obj->setValues('wxappid',\Yii::$app->params['wxPay']['appId']);
        $obj->setValues('send_name','中奥财到家');
        $obj->setValues('re_openid',$openId);
        $obj->setValues('total_amount',$amount*100);
        $obj->setValues('total_num',$num);
        $obj->setValues('wishing',$wishing);
        $obj->setValues('client_ip',$_SERVER['SERVER_ADDR']);
        $obj->setValues('act_name',$act_name);
        $obj->setValues('remark',$remark);
        $res = WxPayApi::SendRedPack($obj);
        return $res;
    }
}
