...
class WxPayApi
{
	...
	public static function unifiedOrder(WxPayUnifiedOrder $inputObj, $timeOut = 6)
	{
        ...
        if(!$inputObj->IsNotify_urlSet()){
            throw new WxPayException("统一支付接口中，异步通知url未设置！");
        }
        if(!$inputObj->GetAppid()){
            throw new WxPayException("统一支付接口中，Appid未设置！");
        }
        if(!$inputObj->GetMch_id()){
            throw new WxPayException("统一支付接口中，Mch_id未设置！");
        }
	...
	public static function orderQuery($inputObj, $timeOut = 6)
	{
		...
		$inputObj->SetAppid(\Yii::$app->params['wxPay']['appId']);//公众账号ID
		$inputObj->SetMch_id(\Yii::$app->params['wxPay']['mchId']);//商户号
		$inputObj->SetNonce_str(self::getNonceStr());//随机字符串
...
//		}
		...
		if($useCert == true){
			...
			curl_setopt($ch,CURLOPT_SSLCERT, \Yii::getAlias(\Yii::$app->params['wxPay']['certFile']));
			curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
			curl_setopt($ch,CURLOPT_SSLKEY, \Yii::getAlias(\Yii::$app->params['wxPay']['keyFile']));
        }
		//post提交方式
	...
	public static function SendRedPack(WxRedPackData $input){
        $url = 'https://api.mch.weixin.qq.com/mmpaymkttransfers/sendredpack';
        $input->setValues('nonce_str',self::getNonceStr());
        $input->SetSign();
        $xml = $input->ToXml();
        $startTimeStamp = self::getMillisecond();//请求开始时间
